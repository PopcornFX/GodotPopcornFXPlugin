...
bool PKBatchDrawerBillboardCPU::Setup(const CRendererDataBase *p_renderer, const CParticleRenderMedium *p_owner, const CFrameCollector *p_fc, const CStringId &p_storage_class) {
...
void PKBatchDrawerBillboardCPU::Destroy() {
...
bool PKBatchDrawerBillboardCPU::AreRenderersCompatible(const CRendererDataBase *p_renderer_a, const CRendererDataBase *p_renderer_b) const {
...
bool PKBatchDrawerBillboardCPU::CanRender(SRenderContext &p_ctx) const {
	if (!Super::CanRender(p_ctx)) {
		return false;
	}

	// TODO: Remove the variation flag check once distortion billboards are implemented
	return shader_mat != nullptr && !shader_mat->has_variation_flag(PKShaderMaterial::VariationFlags::DISTORTION);
}

bool PKBatchDrawerBillboardCPU::AllocBuffers(SRenderContext &p_ctx) {
	if (!PKGD_VERIFY(m_DrawPass != nullptr)) {
		return false;
	}

	PKGD_ASSERT(m_DrawPass->m_TotalParticleCount == m_BB_Billboard.TotalParticleCount());
	particle_count = m_DrawPass->m_TotalParticleCount;
	vtx_count = m_BB_Billboard.TotalVertexCount();
	idx_count = m_BB_Billboard.TotalIndexCount();
	...
	if (realloc_surface) {
		...
		// Resize intermediary buffer of non-packed float4 normals and tangents
		if (nrm_tan_stride > 0) {
			nrm_tan_array.resize(vtx_count_alloc * sizeof(CFloat4) * 2);
		}
	}
...
bool PKBatchDrawerBillboardCPU::MapBuffers(SRenderContext &p_ctx) {
	(void)p_ctx;
	...
	// -----------------------------------------
	if (_should_gen_input(Drawers::GenInput_Indices)) {
		m_BBJobs_Billboard.m_Exec_Indices.m_IndexStream.Setup(idx_array.ptrw(), idx_count, large_indices);
	}
	if (_should_gen_input(Drawers::GenInput_Position)) {
		void *positions_mapped = vtx_array.ptrw();
		TStridedMemoryView<CFloat3> positions_view(static_cast<CFloat3 *>(positions_mapped), vtx_count);
		m_BBJobs_Billboard.m_Exec_PNT.m_Positions = positions_view;
	}
	if (_should_gen_input(Drawers::GenInput_Normal)) {
		void *normals_mapped = nrm_tan_array.ptrw();
		TStridedMemoryView<CFloat3, 0x10> normals_view(static_cast<CFloat3 *>(normals_mapped), vtx_count, 0x10);
		m_BBJobs_Billboard.m_Exec_PNT.m_Normals = normals_view;
	}
	if (_should_gen_input(Drawers::GenInput_Tangent)) {
		void *tangents_mapped = nrm_tan_array.ptrw() + sizeof(CFloat4) * vtx_count_alloc;
		TStridedMemoryView<CFloat4, 0x10> tangents_view(static_cast<CFloat4 *>(tangents_mapped), vtx_count, 0x10);
		m_BBJobs_Billboard.m_Exec_PNT.m_Tangents = tangents_view;
	}
	if (_should_gen_input(Drawers::GenInput_UV0)) {
		...
		m_BBJobs_Billboard.m_Exec_Texcoords.m_Texcoords = texcoords_view;
	}
	if (_should_gen_input(Drawers::GenInput_UV1)) {
		...
		m_BBJobs_Billboard.m_Exec_Texcoords.m_Texcoords2 = texcoords2_view;
	}
	...
	SBillboardBatchJobs::SPerView &cur_view_jobs = m_BBJobs_Billboard.m_PerView[0];
	if (_should_gen_input(Drawers::GenInput_Indices, 0)) {
	...
	if (!m_DrawPass->m_ToGenerate.m_AdditionalGeneratedInputs.Empty()) {
		...
		m_BBJobs_Billboard.m_Exec_CopyField.m_FieldsToCopy = additional_inputs_mapped.View();
		m_BBJobs_Billboard.m_Exec_CopyField.m_PerVertex = false;
	}
	return true;
}

bool PKBatchDrawerBillboardCPU::UnmapBuffers(SRenderContext &p_ctx) {
	...
	if (nrm_tan_stride > 0) {
		...
		for (uint32_t i = 0; i < vtx_count; ++i) {
			const uint32_t read_offset = i * sizeof(CFloat4);

			// Not sure why normals need to be flipped...
			const Vector3 normal = -nrm_tan_array.decode_vector3(read_offset);
			const Vector3 tangent = nrm_tan_array.decode_vector3(tangent_offset + read_offset);
			const float tan_sign = nrm_tan_array.decode_float(tangent_offset + read_offset + sizeof(CFloat3));

			const Vector2 normal_encoded = normal.octahedron_encode();
			const Vector2 tangent_encoded = tangent.octahedron_tangent_encode(tan_sign);
...
bool PKBatchDrawerBillboardCPU::EmitDrawCall(SRenderContext &p_ctx, const SDrawCallDesc &p_to_emit) {
	if (owner_scene == nullptr) {
		return false;
	}
...
String PKBatchDrawerBillboardCPU::_get_additional_stream_offset_name(AdditionalStreamOffsets p_stream) {
	switch (p_stream) {
...
bool PKBatchDrawerBillboardCPU::_should_gen_input(Drawers::EGeneratedInput p_input) const {
	PKGD_ASSERT(m_DrawPass != nullptr);
	return m_DrawPass->m_ToGenerate.m_GeneratedInputs & p_input;
}

bool PKBatchDrawerBillboardCPU::_should_gen_input(Drawers::EGeneratedInput p_input, uint32_t p_view_idx) const {
	PKGD_ASSERT(m_DrawPass != nullptr);
...
bool PKBatchDrawerBillboardCPU::_is_additional_input_supported(const CStringId &p_input_name, EBaseTypeID p_input_type, AdditionalStreamOffsets &r_stream_offset_type) const {
	...
	return r_stream_offset_type != STREAM_OFFSET_MAX;
}

void PKBatchDrawerBillboardCPU::_clear_stream_offsets() {
